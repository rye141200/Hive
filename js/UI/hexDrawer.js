export class hexDrawer {
  ...
  drawByAbsoluteCoordinates(hex) {
    ...
    hexHTML.dataset.stack = hex.stack;
    this.hexContainer.appendChild(hexHTML);
    return hexHTML; //! If code breaks change it to hex only
  }
  drawForAI(hex, color, insect) {
    const hexHTML = document.createElement("div");
    hexHTML.classList.add(this.hexType);
    ...
    hexHTML.dataset.coordinates = hex.coordinates;
    hexHTML.dataset.stack = hex.stack;
    hexHTML.style.background = `url(./assets/${color}_${insect}.png)`;
    this.hexContainer.appendChild(hexHTML);
    return hexHTML; //! If code breaks change it to hex only
  }
  drawInitialHex() {
