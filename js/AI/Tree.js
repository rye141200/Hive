export default class Tree {
  ...
  addNode(node) {
    ...
    this.#tree[node.getId()].setDepth(
      this.#tree[node.getParentId()].getDepth() + 1
    );
  }
  ...
  optimize() {
    if (!this.#isAllEvaluated) this.minimax();
    ...
    Object.keys(this.#tree).forEach((nodeId) => {
      if (this.#tree[nodeId].getChildrenIds().length > 1) {
        ...
        // minimizer
        else this.#orderChildrenAscending(this.#tree[nodeId]);
      }
    });
  }
  ...
  #orderChildrenAscending(node) {
    ...
    for (let i = 0; i < node.getChildrenIds().length; i++) {
      for (let j = 0; j < node.getChildrenIds().length - i; j++) {
        ...
        minEvaluation =
          this.#tree[node.getChildrenIds()[minIndex]].getEvaluation();
  ...
  #orderChildrenDescending(node) {
    ...
    for (let i = 0; i < node.getChildrenIds().length; i++) {
      for (let j = 0; j < node.getChildrenIds().length - i; j++) {
        childjEvaluation = this.#tree[node.getChildrenIds()[j]].getEvaluation();
        maxEvaluation =
          this.#tree[node.getChildrenIds()[maxIndex]].getEvaluation();
  ...
  minimax(nodeId = this.#rootNodeId, nodeAlpha = -999, nodeBeta = 999) {
    ...
    if (nodeChildernIds.length === 0) {
      ...
      for (let i = 0; i < nodeChildernIds.length; i++) {
        let returnedEvaluation = this.minimax(
          nodeChildernIds[i],
          node.getAlpha(),
          node.getBeta()
        );
  ...
  minimaxWithPruning(
    ...
    if (nodeChildernIds.length === 0) {
      ...
      for (let i = 0; i < nodeChildernIds.length; i++) {
        let returnedEvaluation = this.minimaxWithPruning(
          nodeChildernIds[i],
          node.getAlpha(),
          node.getBeta()
        );
        ...
        if (node.getDepth() % 2 === 0) {
          if (node.getAlpha() < returnedEvaluation) {
            ...
            if (
              parentNodeId !== null &&
              node.getAlpha() >= this.#tree[parentNodeId].getBeta()
            ) {
              break;
            }
          }
        ...
        // minimizer
          ...
          if (node.getBeta() > returnedEvaluation) {
            ...
            if (
              parentNodeId !== null &&
              node.getBeta() <= this.#tree[parentNodeId].getAlpha()
            ) {
              break;
            }
          }
